...
layui.define([ 'jquery','layer'], function(exports) {
	...
	layui.link((layui.cache.base?layui.cache.base:"")+'ztree/ztree.css', "ztreecss");
	// 1>:构造类..................................
	/**
	 * 构造类方法
	 */
	var Tree = function Tree(element, options) {
		...
		// 设置对象属性.
		if (typeof element === 'string' || !$.isPlainObject(element)) {
		...
		// 获得缓存对象
		var instance=$(this.element).data("tree");
		if(instance){
	...
	Tree.prototype.initialize = function() {
		...
		if(typeof options.onLoadBefore === 'function'){
			options.onLoadBefore.call(this);
		}
		
		var id=$(this.element).attr('id');
		if(!id){
			id='tree_'+new Date().getTime();
		...
		this.setting = {
            ...
            data: {
            	key: {
    				...
    				isParent: options.isParent || 'isParent',
    				isHidden: options.isHidden || 'isHidden',
    			},
                simpleData: {
                    enable: true,  
                	idKey : options.id,
            ...
            callback: {
            	// 单击事件
            	onClick:function(event,treeId,node,cflag){
            		if(typeof options.onClick === 'function'){
            			options.onClick.call($this,node,event);
            		}
            	},
            	onDblClick:function(event,treeId,node){
            		if(typeof options.onDblClick === 'function'){
            			options.onDblClick.call($this,node,event);
            		}
            	},
            	onCheck:function(event,treeId,node){
            		if(typeof options.onCheck === 'function'){
            			options.onCheck.call($this,node,event);
            		}
            	},
            	onCollapse:function(event,treeId,node){
            		if(typeof options.onCollapse === 'function'){
            			options.onCollapse.call($this,node,event);
            		}
            	},
            	onRightClick:function(event,treeId,node){
            		if(typeof options.onRightClick === 'function'){
            			options.onRightClick.call($this,node,event);
            		}
            	},
            	onNodeCreated:function(event,treeId,node){
            		if(typeof options.onNodeCreated === 'function'){
            			options.onNodeCreated.call($this,node,event);
            		}
            	},
            	// before 事件
            	beforeCheck:function(treeId, node){
            		if(typeof options.beforeCheck === 'function'){
            			return options.beforeCheck.call($this,node);
            		}
            	},
            	// 异步数据加载异常事件
            	onAsyncError:function(event, treeId, treeNode, XMLHttpRequest, textStatus, errorThrown){
            		if(typeof options.onLoadError === 'function'){
            			options.onLoadError.call($this,treeNode,event,XMLHttpRequest, textStatus, errorThrown);
            		}
            	},
            	// 异步数据加载成功事件
            	onAsyncSuccess:function(event, treeId, treeNode, msg){
            		if(typeof options.onLoadSuccess === 'function'){
            			options.onLoadSuccess.call($this,treeNode,msg,event);
            		}
            	}
            },
            // 选择配置
        	...
        	async: {
        		...
        		dataFilter: function(treeId, parent, data){
        			if(data && options.root && typeof data === 'object' && data[options.root]){
        				data=data[options.root];
        			}
        			if(typeof options.doDataFilter === 'function'){
        				var temp=options.doDataFilter.call($this,parent, data);
		...
		this.target=$.fn.zTree.init($(this.element), $.extend(true,this.setting,options.setting), options.data);
	}
	 ...
	 * 设置选中节点
	 */
	Tree.prototype.selectNode=function(node,flag){
		this.target.selectNode(node,flag);
	}
	Tree.prototype.cancelSelectedNode=function(node){
		this.target.cancelSelectedNode(node);
	}
	/**
	 * 获得选中节点
	 * @returns
	 */
	Tree.prototype.getSelectNodes=function(){
		return this.target.getSelectedNodes();
	}
	...
	Tree.prototype.getNodes=function(){
		return this.target.getNodes();
	}
	
	Tree.prototype.getNodeById=function(nodeId){
		return this.target.getNodeByTId(nodeId);
	}
	
	Tree.prototype.getNodeIndex=function(node){
		return this.target.getNodeIndex(node);
	}
	...
	Tree.prototype.getCheckedNodes=function(flag){
		return this.target.getCheckedNodes(flag);
	}
	 ...
	 * @returns
	 */
	Tree.prototype.getChangeCheckedNodes=function(){
		return this.target.getChangeCheckedNodes();
	}
	
	Tree.prototype.addNodes=function(parent,nodes,flag){
		return this.target.addNodes(parent,nodes,flag);
	}
	Tree.prototype.updateNode=function(nodes,flag){
		this.target.updateNode(nodes,flag);
	}
	Tree.prototype.removeNode=function(nodes,flag){
		this.target.removeNode(nodes,flag);
	}
	Tree.prototype.removeChildNodes=function(parent){
		this.target.removeChildNodes(parent);
	}
	
	Tree.prototype.setChkDisabled=function(nodes,flag,parentCascade,childrenCascade){
		this.target.setChkDisabled(nodes,flag,parentCascade,childrenCascade);
	}
	
	
	Tree.prototype.showNode=function(node){
		this.target.showNode(node);
	}
	Tree.prototype.showNodes=function(nodes){
		this.target.showNodes(nodes);
	}
	
	Tree.prototype.hideNode=function(node){
		this.target.hideNode(node);
	}
	Tree.prototype.hideNodes=function(nodes){
		this.target.hideNodes(nodes);
	}
	...
	Tree.prototype.checkAllNodes=function(flag){
		this.target.checkAllNodes(flag);
	}
	 ...
	 * @param flag
	 * @param parentCascade
	 * @param childrenCascade
	 */
	Tree.prototype.checkNode=function(node,flag,parentCascade,childrenCascade){
		this.target.checkNode(node,flag,parentCascade,childrenCascade);
	}
	 ...
	 * @param flag
	 */
	Tree.prototype.moveNode=function(node,targetNode,moveType,flag){
		this.target.moveNode(targetNode,node,moveType,flag);
	}
	 ...
	 * @param flag
	 */
	Tree.prototype.copyNode=function(node,targetNode,moveType,flag){
		this.target.copyNode(targetNode,node,moveType,flag);
	}
	 ...
	 * @param flag
	 */
	Tree.prototype.setEditable=function(flag){
		this.target.setEditable(flag);
	}
	
	/**
	 * 据自定义规则搜索节点数据 JSON 对象集合 或 单个节点数据
	 * 可自定义复杂的搜索规则
	 * @param filter
	 * @param isSingle
	 * @param parent
	 * @param params
	 */
	Tree.prototype.getNodesByFilter=function(filter,isSingle,parent,params){
		return this.target.getNodesByFilter(filter,isSingle,parent,params);
	}
	 ...
	 * @returns
	 */
	Tree.prototype.getNodesByParam=function(key,value,parent){
		return this.target.getNodesByParam(key,value,parent);
	}
	 ...
	 * @returns
	 */
	Tree.prototype.getNodesByParamFuzzy=function(key,value,parent){
		return this.target.getNodesByParamFuzzy(key,value,parent);
	}
	...
	Tree.prototype.reloadNodes=function(nodes){
		if(nodes){
			this.setting.async.url=null;
		}
		this.destroy();
		this.target=$.fn.zTree.init($(this.element), $.extend(true,{},this.setting,this.options.setting), nodes);
	}
	
	
	// 4>:默认参数........
	Tree.defaults = {
		prefix: 'tree',	 		// 唯一主键前缀名称.
		showLine: true,  
        showIcon: true,
        checkable:true,
        chkboxType: { "Y" : "ps", "N" : "s" },
        ...
        onNodeCreated:function(node,event){},
        // tree树 do事件
        doDataFilter:function(parent,data){},
        // tree树 before事件
